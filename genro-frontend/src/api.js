...
// Syllabus changes rarely, while students revisit it often. Keep completed
// responses briefly and share an in-flight request to avoid duplicate fetches.
const syllabusCache = new Map();
const SYLLABUS_CACHE_TTL_MS = 10 * 60 * 1000;
...
export const api = {
  ...
  getSyllabus: (classLevel, subject, { force = false } = {}) => {
    const key = `${classLevel}:${subject}`;
    const cached = syllabusCache.get(key);
    if (!force && cached && (cached.promise || Date.now() - cached.savedAt < SYLLABUS_CACHE_TTL_MS)) return cached.promise || Promise.resolve(cached.value);
    const promise = request(`/api/syllabus/${toPathSegment(classLevel)}/${toPathSegment(subject)}`)
      .then((value) => {
        syllabusCache.set(key, { value, savedAt: Date.now() });
        return value;
      })
      .catch((error) => { syllabusCache.delete(key); throw error; });
    syllabusCache.set(key, { promise, savedAt: Date.now() });
    return promise;
  },

  getTest: (id, kind = 'topic', difficulty = 'all') => {

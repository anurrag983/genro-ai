...
function ChapterDetailView({ chapter, onBack, onStartQuiz }) {
  ...
  return <section className="chapter-detail">
    <div className="chapter-breadcrumb">
      <button className="chapter-breadcrumb-back" type="button" onClick={onBack}>
        <ArrowLeft size={14} />
        <span>All Chapters</span>
      </button>
      <span className="chapter-breadcrumb-sep">/</span>
      <span className="chapter-breadcrumb-current">{chapter.chapter_name}</span>
    </div>
...
function CustomPracticePage({ user, onStartQuiz }) {
  ...
  return (
    <div className="page-stack custom-practice-page">
      ...
      {/* ── SEARCH ONLY (difficulty moved to sticky bar) ── */}
      <div className="cp-search-box cp-search-full">
        <Search size={16} />
      ...
      {selectedCount > 0 && (
        <div className="custom-sticky-bar">
          <div className="custom-sticky-info">
            <span className="custom-sticky-count">{selectedCount} topic{selectedCount === 1 ? '' : 's'}</span>
            <span className="custom-sticky-hint">Choose difficulty:</span>
          </div>
          <div className="sticky-diff-row" role="radiogroup" aria-label="Difficulty">
            {[{id:'all',label:'All',color:'purple'},{id:'easy',label:'Easy',color:'green'},{id:'medium',label:'Medium',color:'amber'},{id:'tough',label:'Hard',color:'rose'}].map(({id, label, color}) => (
              <button key={id} type="button"
                className={`sticky-diff-pill ${color} ${difficulty === id ? 'active' : ''}`}
...
function QuizResults({ descriptor, questions, answers, score, accuracy, elapsedSeconds, state, onBack }) {
  ...
  return <div className="quiz-results">
      ...
      <div className="result-right-col">
        ...
        <div className="result-actions">
          ...
          <button className="secondary-button compact" type="button" onClick={() => {
            ...
            setShowReport(next);
            if (next) setTimeout(() => reportRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
          }}>
    ...
    {showReport && <div className="result-report-drawer" ref={reportRef}>
